...
class SchemaExporter:
    ...
    def export_schema_to_json(self, schema_name):
        """
        Exporte les métadonnées du schéma en format JSON
        :param schema_name: Nom du schéma à exporter
        :return: Dictionnaire JSON contenant les métadonnées du schéma
        """
        metadata = self.get_schema_metadata(schema_name)

        try:
            # Convertir en JSON et retourner directement
            json_data = json.loads(
                json.dumps(metadata, indent=2, ensure_ascii=False, default=str)
            )
            logger.info(f"Export JSON réussi pour le schéma {schema_name}")
            return json_data
    ...
    def _get_tables_metadata(self, schema_name):
        ...
        try:
            ...
            cursor.execute(
                ...
                FROM all_tables 
                WHERE owner = :owner 
                AND table_name NOT LIKE 'REJECTED%'
                ORDER BY table_name
            """,
                {"owner": schema_name},
            )
